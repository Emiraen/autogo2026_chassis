...
void OLED_HorizontalShift(uint8_t direction) {
    WriteCmd(0x2E);   // 停止滚动，0x2E：停止
    WriteCmd(direction); // 设置滚动方向
    WriteCmd(0x00); // 虚拟字节设置，默认为 0x00
    WriteCmd(0x00); // 设置开始页地址
    WriteCmd(0x05); // 设置每个滚动步骤之间的时间间隔的帧频
    WriteCmd(0x07); // 设置结束页地址
    WriteCmd(0x00); // 虚拟字节设置，默认为 0x00
    WriteCmd(0xFF); // 虚拟字节设置，默认为 0xFF
    WriteCmd(
      0x2F); // 启动滚动，0x2F：启动
}
void StartScrollRight(void) {
    OLED_HorizontalShift(0x26); // 0x26 启动右滚动
}
void StartScrollLeft(void) {
    OLED_HorizontalShift(0x27); // 0x27 启动左滚动
}
void StopScroll(void) {
    WriteCmd(0x2E); 
}

/* IMU 显示功能已移至 main.c 的 Task_OLED_20Hz() */
